...
def start_event_worker(device, start_time, event_log):
    return subprocess.Popen([
        "sudo", sys.executable, str(Path(__file__).resolve()), "--worker",
        "--device", device, "--start-time", str(start_time), "--log", str(event_log)
...
def get_session_events(events, start_time, end_time):
    return [
        (max(0.0, timestamp - start_time), event)
...
def find_skill_circle(frame):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, dp=1.2, minDist=45,
                               param1=100, param2=27,
    ...
    for x, y, radius in circles[0]:
        ...
        if x < radius or y < radius or x >= frame.shape[1] - radius or y >= frame.shape[0] - radius:
            continue
        candidates.append((x, y, radius))
    if not candidates:
        return None
    return max(candidates, key=lambda c: circle_score(frame, c))
...
def circle_score(frame, circle):
    ...
    valid = ((xs >= 1) & (xs < frame.shape[1] - 1) & (ys >= 1) & (ys < frame.shape[0] - 1))
        ...
        if circle is not None:
            ...
            if active:
                ...
                if misses >= END_MISSES:
                    ...
                    detections.append({"start": max(0.0, start_frame / fps), "end": max(0.0, end_frame / fps)})
                    active = False
                    start_frame = None
                    last_hit_frame = None
                    misses = 0
...
def make_subtitles(events, skill_checks, path):
    ...
    lines = ["[Script Info]", "ScriptType: v4.00+", "PlayResX: 320", "PlayResY: 240", "",
             "[V4+ Styles]",
        ...
        try:
            while True:
                ...
                if not recording and lmb_state and current_down is not None:
                    ...
                    print()
                    print(f"ЛКМ зажата → НАЧАЛО СБОРА: {output.name}")
                    print()
                    recorder = subprocess.Popen([
                        "gpu-screen-recorder", "-w", REGION, "-f", str(FPS),
                        "-c", "mkv", "-o", str(raw_video)
                    ], preexec_fn=lambda: signal.signal(signal.SIGINT, signal.SIG_IGN))
                ...
                if recorder is not None and recorder.poll() is not None:
                    print("ОШИБКА: gpu-screen-recorder неожиданно завершился.")
                    recorder = None
                    recording = False
                    session_start = None
                    output = None
