...
def save_report(report):
    ...
    parts=[text[i:i+REPORT_LIMIT] for i in range(0,len(text),REPORT_LIMIT)] or ['']
    existing=[]
    ...
    print(f'Отчёт сохранён: {len(parts)} файл(ов), максимум {REPORT_LIMIT} символов каждый')
...
def circle_candidates(frame):
    gray=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY); gray=cv2.GaussianBlur(gray,(7,7),1.5)
...
def ring_features(frame,cx,cy,r,r0=-5,r1=8,bins=144):
    hsv=cv2.cvtColor(frame,cv2.COLOR_BGR2HSV); s=hsv[:,:,1].astype(np.float32); v=hsv[:,:,2].astype(np.float32)
    h,w=v.shape; yy,xx=np.indices((h,w)); rr=np.sqrt((xx-cx)**2+(yy-cy)**2); keep=(rr>=r+r0)&(rr<=r+r1)
    if keep.sum()<100:return None
    ang=(np.arctan2(-(yy-cy),xx-cx)+2*np.pi)%(2*np.pi); bi=(ang[keep]*bins/(2*np.pi)).astype(np.int32)
    def hist(score):
        z=score[keep]; sm=np.bincount(bi,weights=z,minlength=bins); n=np.bincount(bi,minlength=bins); return sm/np.maximum(n,1)
    sv=s[keep]; vv=v[keep]; qv=np.percentile(vv,[20,35,50,65,80]); qs=np.percentile(sv,[20,50,80])
...
def candidate_score(frame,c):
    ...
    return max(ws,bs)+0.35*rs+0.15*max(wp,bp),(wa,ws,wp,ba,bs,bp,ra,rs,rp)

def local_check(cap,fps,start,end,sample):
    cap.set(cv2.CAP_PROP_POS_MSEC,max(0,start*1000)); rows=[]; step=max(1,round(fps/sample)); frame_i=int(max(0,round(start*fps))); end_i=int(end*fps); last=None
    while frame_i<=end_i:
        ok,frame=cap.read()
        if not ok:break
        if (frame_i-int(round(start*fps)))%step!=0: frame_i+=1; continue
        cs=circle_candidates(frame); ranked=[]
        for c in cs:
            sc,feat=candidate_score(frame,c)
            if sc<0:continue
            if last is not None:
                lx,ly,lr=last; x,y,r=c; sc-=0.08*(abs(x-lx)+abs(y-ly))+0.04*abs(r-lr)
        ...
        if ranked:
            ...
            if last is None or math.hypot(c[0]-last[0],c[1]-last[1])<45:
                last=c; rows.append({'t':frame_i/fps,'x':c[0],'y':c[1],'r':c[2],'score':sc,'white_angle':feat[0],'white_strength':feat[1],'white_peak':feat[2],'black_angle':feat[3],'black_strength':feat[4],'black_peak':feat[5],'red_angle':feat[6],'red_strength':feat[7],'red_peak':feat[8]})
        frame_i+=1
    return rows

def angular_fit(rows,key,space):
    if len(rows)<6:return None
    t=np.array([x['t'] for x in rows],float); a=np.unwrap(np.array([x[key] for x in rows],float)); keep=t<=space+0.02; t=t[keep]; a=a[keep]
...
def summarize(rows,space):
    ...
    wf=angular_fit(rows,'white_angle',space); bf=angular_fit(rows,'black_angle',space); last=rows[-1]; result='UNKNOWN'; chosen=None
...
def main():
    ...
    for i,s in enumerate(spaces,1):
        start=max(0,s-1.8);end=s+0.08;rows=local_check(cap,fps,start,end,a.sample_fps);z=summarize(rows,s);z['space']=s;z['index']=i;z['trajectory']=rows;checks.append(z);c=z.get('center_mean');print(f"#{i} SPACE={s:.3f} frames={z['frames']} center="+('%.1f,%.1f'%tuple(c) if c else 'none')+f" -> {z['result']}")
